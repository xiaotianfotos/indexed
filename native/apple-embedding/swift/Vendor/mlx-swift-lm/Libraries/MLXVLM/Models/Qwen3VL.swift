...
extension Qwen3VLLanguage {
    ...
    static func getRopeIndex(
        ...
        // Qwen3VL/Qwen3.5 prompts wrap each temporal video pair in its own
        // timestamped vision block. The Python runtime therefore repeats a
        // [t,h,w] video grid t times and changes every repeated t to 1 before
        // constructing MRoPE positions.
        let splitVideoGridTHW = videoGridTHW?.flatMap { grid in
            (0 ..< grid.t).map { _ in THW(1, grid.h, grid.w) }
        }
        ...
        for batchIdx in 0 ..< batchSize {
            ...
            // Count vision blocks, not pad tokens. A video frame block has 196
            // consecutive video tokens but exactly one vision_start token.
            let visionStarts = inputTokens.indices.dropLast().filter {
                inputTokens[$0] == visionStartTokenId
            }
            let imageNums = visionStarts.count {
                inputTokens[$0 + 1] == imageTokenId
            }
            let videoNums = visionStarts.count {
                inputTokens[$0 + 1] == videoTokenId
            }
            ...
            for _ in 0 ..< (imageNums + videoNums) {
                ...
                if edImage < edVideo {
                    ...
                    // Process video
                    guard let grid = splitVideoGridTHW, videoIndex < grid.count else { break }
