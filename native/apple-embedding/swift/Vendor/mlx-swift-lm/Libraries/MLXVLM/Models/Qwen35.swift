...
public enum Qwen35Language {
    ...
    final class GatedDeltaNet: Module {
        var embeddingPrefillBackend: GatedDeltaPrefillBackend?
        let hiddenSize: Int
        let numVHeads: Int
        let numKHeads: Int
        let headKDim: Int
        ...
        func callAsFunction(
            ...
            (out, state) = gatedDeltaUpdate(
                ...
                state: state,
                mask: mask,
                prefillBackend: embeddingPrefillBackend
            )
    ...
    final class Model: Module {
        ...
        func callAsFunction(
            ...
            precondition(startLayer >= 0 && startLayer <= layers.count)
            for index in startLayer ..< layers.count {
    ...
    /// Language-only Qwen3.5 backbone API used by WeMM embedding runtimes.
    ///
    /// This intentionally avoids allocating the MLX vision tower or vocabulary
    /// projection. Images are encoded by Core ML and injected as `imageFeatures`.
    public final class LanguageModel: Module, BaseLanguageModel {
        ...
        /// Indexed maintainer hook after q/k normalization and decay/beta setup.
        /// Pass the real decoder layer index; requests never infer it from call order.
        public func setEmbeddingPrefillBackend(
            _ backend: @escaping (Int, MLXArray, MLXArray, MLXArray, MLXArray, MLXArray, MLXArray, MLXArray?) -> (MLXArray, MLXArray)?
        ) {
            for (index, layer) in model.layers.enumerated() {
                layer.linearAttn?.embeddingPrefillBackend = { q, k, v, g, beta, state, mask in
                    backend(index, q, k, v, g, beta, state, mask)
                }
            }
        }

        public init(_ config: Qwen35Configuration, embeddingOnly: Bool = false) {
            self.config = config
            ...
            if !embeddingOnly && !config.textConfiguration.tieWordEmbeddings {
        ...
        /// Build token embeddings and multimodal position IDs without running a decoder layer.
        public func prepareEmbeddingInputs(
            inputIds: MLXArray,
            imageFeatures: MLXArray? = nil,
            imageGridTHW: [THW]? = nil,
            videoGridTHW: [THW]? = nil
        ) throws -> (embeddings: MLXArray, positionIds: MLXArray) {
            let inputIds = inputIds.ndim == 1 ? inputIds.expandedDimensions(axis: 0) : inputIds
            var embeddings = model.embedTokens(inputIds)
            if let imageFeatures {
                let imageMask = inputIds .== MLXArray(config.imageTokenId)
                let videoMask = inputIds .== MLXArray(config.videoTokenId)
                let specialMask = imageMask .|| videoMask
                let tokenCount = specialMask.sum().item(Int.self)
                guard tokenCount == imageFeatures.dim(0) else {
                    throw Qwen35VLError.featureTokenMismatch(
                        expected: tokenCount, actual: imageFeatures.dim(0))
                }
                let expandedMask = broadcast(
                    specialMask[0..., 0..., .newAxis], to: embeddings.shape)
                let flatMask = expandedMask.flattened().asArray(Bool.self)
                let indices = flatMask.enumerated().compactMap { $0.element ? UInt32($0.offset) : nil }
                let flattenedFeatures = imageFeatures.flattened()
                guard indices.count == flattenedFeatures.size else {
                    throw Qwen35VLError.featureTokenMismatch(
                        expected: tokenCount, actual: imageFeatures.dim(0))
                }
                var flattened = embeddings.flattened()
                flattened[MLXArray(indices)] = flattenedFeatures
                embeddings = flattened.reshaped(embeddings.shape)
            }
            ...
            return (embeddings, positionIds)
        }

        /// Run the language backbone and return final hidden states, never logits.
        public func embeddingHiddenStates(
            inputIds: MLXArray,
            imageFeatures: MLXArray? = nil,
            imageGridTHW: [THW]? = nil
        ) throws -> MLXArray {
            let prepared = try prepareEmbeddingInputs(
                inputIds: inputIds,
            ...
            return model(
                inputIds,
                inputsEmbeds: prepared.embeddings,
                positionIds: prepared.positionIds
            )
        }

        /// Continue after a Core ML decoder prefix. `hiddenStates` must use the
        /// same sequence length and `positionIds` returned by `prepareEmbeddingInputs`.
        public func embeddingHiddenStates(
            inputIds: MLXArray,
            ...
            model(
                inputIds,
                inputsEmbeds: hiddenStates,
                positionIds: positionIds,
                startLayer: startLayer
            )
        }

        /// Apply only the final language RMSNorm after a complete external decoder.
        ///
        /// Unlike `embeddingHiddenStates(..., startLayer: 24)`, this deliberately
        /// avoids constructing attention and SSM masks.  It is used by Core ML
        /// decoder bundles whose L0-L23 output is already complete.
        public func finalizeEmbeddingHiddenStates(_ hiddenStates: MLXArray) -> MLXArray {
            model.norm(hiddenStates)
        }

        /// Materialize the dynamic multimodal RoPE values used by full-attention
        /// decoder layers.  Core ML decoder-prefix runtimes use this to preserve
        /// the exact Qwen3.5 position semantics across the MLX/Core ML boundary.
        public func embeddingRotaryValues(
            hiddenStates: MLXArray,
            positionIds: MLXArray
        ) -> (cos: MLXArray, sin: MLXArray) {
            let headDim = textConfig.headDim
                ?? (textConfig.hiddenSize / textConfig.attentionHeads)
            let rotaryDim = Int(Float(headDim) * textConfig.partialRotaryFactor)
            let mrope = textConfig.ropeParameters?["mrope_section"]?.asInts()
                ?? [11, 11, 10]
            let rotary = RotaryEmbedding(
                dim: rotaryDim,
                base: textConfig.ropeTheta,
                mropeSection: mrope
            )
            return rotary(x: hiddenStates, positionIds: positionIds)
        }

        public func sanitize(weights: [String: MLXArray]) -> [String: MLXArray] {
            weights.filter { !$0.key.contains("mtp.") && !$0.key.hasPrefix("lm_head.") }
        }
        ...
        public func sanitize(
            ...
            var result = sanitize(weights: weights)
            let shouldShift = result.contains {
            ...
            for key in Array(result.keys) {
                ...
                if key.contains("conv1d.weight") && value.dim(-1) != 1 {
                    result[key] = value.movedAxis(source: 2, destination: 1)
                } else if shouldShift && value.ndim == 1
