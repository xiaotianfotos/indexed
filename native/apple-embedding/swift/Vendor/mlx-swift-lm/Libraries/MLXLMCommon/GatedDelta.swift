...
/// Optional embedding-only recurrence hook. Returning nil preserves the normal
/// GPU/ops fallback. Indexed keeps this unset for public A/B and all LLM callers.
public typealias GatedDeltaPrefillBackend = (MLXArray, MLXArray, MLXArray, MLXArray, MLXArray, MLXArray, MLXArray?) -> (MLXArray, MLXArray)?
...
public func gatedDeltaUpdate(
    ...
    state: MLXArray? = nil,
    mask: MLXArray? = nil,
    prefillBackend: GatedDeltaPrefillBackend? = nil
) -> (MLXArray, MLXArray) {
    ...
    if let prefillBackend, let result = prefillBackend(q, k, v, g, beta, state, mask) {
        return result
    }
