...
(() => {
  const CONTENT_SCRIPT_VERSION = "0.14.6";
  ...
  // WeMM samples video at 2 fps and pairs adjacent frames into temporal
  // patches. Sending those ordered frames avoids browser recording, WebM
  // repair, MP4 transcoding and a second decode pass in the native backend.
  const FRAME_PACKET_FPS = 2;
  ...
  function captureFps() {
    return runtime.framePackets ? FRAME_PACKET_FPS : PRODUCT_DEFAULTS.capture.framesPerSecond;
  }
  ...
  function publicState() {
    ...
    return {
      ...
      visualInput: runtime.framePackets ? "ordered_frames" : "native_video",
      captureTransport: runtime.framePackets ? "ordered-frames" : "webm",
  ...
  function syncVideoSession(force = false) {
    ...
    if (!force && !runtime.optedIn && !runtime.running) return Promise.resolve(null);
    const now = Date.now();
  ...
  async function finalizeActiveSegment(reason) {
    ...
    runtime.activeSegment = null;
    const blob = segment.recorder ? await stopRecorder(segment) : null;
    const capturedSeconds = Math.max(
      0,
      Number(segment.lastVideoTime || 0) - Number(segment.firstVideoTime || 0) + 1 / captureFps()
    );
    const expectedSeconds = targetSegmentDuration(segment);
    const requiredSeconds = Math.max(0.75, expectedSeconds * 0.8);
    if (
      capturedSeconds < requiredSeconds
      || segment.frameCount < 3
      || (runtime.framePackets ? segment.frames.length < 3 : !blob || blob.size < 1024)
    ) {
      runtime.skippedVisual += 1;
      return { skipped: true, reason: `incomplete_${reason}`, capturedSeconds, expectedSeconds };
    }

    const videoBase64 = blob ? await blobToBase64(blob) : "";
    ...
    const response = await send({
      ...
      payload: {
        ...
        segmentInterval: SEGMENT_SECONDS,
        videoSegmentSchema: runtime.framePackets ? SEGMENT_SCHEMAS.videoFrames : SEGMENT_SCHEMAS.video,
        transcriptSegmentSchema: TRANSCRIPT_SEGMENT_SCHEMA,
        videoFrames: runtime.framePackets ? segment.frames : [],
        videoBase64,
        videoFilename: videoBase64
          ? `${segment.metadata.sourceSite}-${segment.metadata.videoId}-${segment.segmentIndex}.webm`
          : "",
        videoMimeType: segment.mimeType || "",
  ...
  async function startSegment(video, segmentIndex) {
    ...
    if (!context) throw new Error("当前浏览器无法创建视频抽帧画布");
    const segment = {
      metadata,
      segmentIndex,
      canvas,
    ...
    if (!runtime.framePackets) {
      segment.stream = canvas.captureStream(captureFps());
      const mimeType = recorderMimeType();
      const options = { videoBitsPerSecond: VIDEO_BITS_PER_SECOND };
      if (mimeType) options.mimeType = mimeType;
      segment.recorder = new MediaRecorder(segment.stream, options);
      segment.mimeType = segment.recorder.mimeType || mimeType || "video/webm";
      segment.recorder.addEventListener("dataavailable", (event) => {
        if (event.data?.size) segment.chunks.push(event.data);
      });
      segment.recorder.addEventListener("error", (event) => {
  ...
  function drawFrame(segment, video) {
    ...
    if (currentTime - segment.lastDrawVideoTime < 1 / captureFps() - 0.03) return;
    segment.context.drawImage(video, 0, 0, segment.width, segment.height);
    if (runtime.framePackets) {
      segment.frames.push({
        imageBase64: segment.canvas.toDataURL("image/jpeg", 0.72).split(",", 2)[1] || "",
        mimeType: "image/jpeg",
        timestamp: Math.max(0, currentTime - segment.segmentIndex * SEGMENT_SECONDS),
      });
    }
  ...
  async function start(options = {}) {
    const captureConfig = await send({ type: "GET_CAPTURE_CONFIG" });
    runtime.framePackets = Boolean(captureConfig?.framePackets);
  ...
  document.addEventListener("visibilitychange", () => {
    ...
    // Indexed never owns playback. Returning to the tab only refreshes capture
    // state; the player remains entirely under the user's control.
    void syncRebuildJob();
  });
