...
import { queueStatusPresentation } from "./queue-state.js";
import { backendModePresentation, saveBeforeConnectionTest } from "./settings-flow.js";
...
function setBackendMode(mode) {
  const presentation = backendModePresentation(mode);
  backendMode = presentation.mode;
  elements["backend-cloud"].checked = backendMode === "cloud";
  elements["backend-local"].checked = backendMode === "local";
  elements["cloud-settings"].hidden = backendMode !== "cloud";
  elements["local-settings"].hidden = backendMode !== "local";
  elements["cost-estimate"].hidden = backendMode !== "cloud";
...
function selectBackendMode(mode) {
  setBackendMode(mode);
  elements["settings-status"].textContent = backendModePresentation(mode).selected;
}
...
function renderHistory() {
  ...
  for (const video of videos) {
    ...
    } else if (video.processing || video.pending_count > 0) {
      action = document.createElement("span");
      action.className = "video-state processing";
      action.textContent = video.processing ? "正在处理" : "队列中";
...
function renderDashboard() {
  ...
  const queuePresentation = queueStatusPresentation(queueState);
  elements["queue-status"].hidden = !queuePresentation.visible;
  elements["queue-status"].className = `queue-status ${queuePresentation.tone}`;
  elements["queue-status"].textContent = queuePresentation.text;
  elements["queue-status"].title = queuePresentation.title;
...
async function refreshStats() {
  ...
  if (library.error) {
    ...
    elements.connection.textContent = backendMode === "local"
      ? "本地未连接"
      : (detail.includes("Embedding") ? "模型未连接" : "云端未连接");
...
function formConfig() {
  const controlPlaneUrl = backendMode === "local"
    ? normalizeControlPlaneUrl(elements["control-plane-url"].value)
    : elements["control-plane-url"].value.trim().replace(/\/+$/, "");
  return {
    backendMode,
    controlPlaneUrl,
...
async function discoverEmbedding({ force = false } = {}) {
  ...
  try {
    const origin = httpHostPermissionPattern(baseUrl);
...
async function ensureConnectionPermissions(config) {
  try {
    const origins = config.backendMode === "local"
      ? [httpHostPermissionPattern(config.controlPlaneUrl)]
      : [httpHostPermissionPattern(config.embeddingBaseUrl), `https://${vectorEndpoint(config)}/*`];
...
async function persistSettings(config, { automatic = false } = {}) {
  ...
  validatedConfigFingerprint = "";
  elements["settings-status"].textContent = config.backendMode === "local"
    ? `本地设置已保存 · ${new URL(config.controlPlaneUrl).host}`
    : "阿里云设置已保存";
  await refreshStats();
  elements["save-settings"].disabled = false;
  return true;
...
async function validateSettings(config, {
  ...
  try {
    if (!await ensureConnectionPermissions(config)) {
      if (alreadySaved) {
        const detail = elements["settings-status"].textContent;
        elements["settings-status"].textContent = `设置已保存，但${detail}`;
      }
      return false;
    }
    ...
    const model = tested.embedding?.model
      ...
      || "Embedding 在线";
    const storage = tested.storage?.provider_label || tested.storage?.provider || (config.backendMode === "local" ? "本地 zvec" : "向量库在线");
...
async function saveSettings() {
  ...
  try {
    config = formConfig();
    if (config.backendMode === "local") elements["control-plane-url"].value = config.controlPlaneUrl;
  } catch (error) {
    elements["settings-status"].textContent = String(error.message || error);
    return;
  }
  await saveBeforeConnectionTest(
    () => persistSettings(config),
    () => validateSettings(config, { alreadySaved: true }),
  );
}
...
async function openLocalFrontend(view) {
  let url;
  try {
    url = normalizeControlPlaneUrl(elements["control-plane-url"].value);
  } catch {
    elements["settings-panel"].hidden = false;
...
elements.library.addEventListener("click", () => void chrome.tabs.create({ url: chrome.runtime.getURL("search.html") }));
elements["local-library"].addEventListener("click", () => void openLocalFrontend("files"));
elements["backend-cloud"].addEventListener("change", () => selectBackendMode("cloud"));
elements["backend-local"].addEventListener("change", () => selectBackendMode("local"));
