...
function openDeleteDialog(video) {
  ...
  elements["delete-dialog-summary"].textContent = `${video.visual_count || 0} 条画面索引 · ${video.transcript_count || 0} 条字幕索引 · 共 ${total} 条索引记录`;
...
function createLibraryCard(video) {
  ...
  state.textContent = video.processing ? "正在处理" : video.failed_count ? `${video.failed_count} 个分片失败` : "已保存";
...
async function refreshLibrary() {
  ...
  elements.connection.title = response.storage_label || "Indexed 存储后端";
...
async function deleteVideo() {
  ...
  elements["delete-dialog-status"].textContent = "正在停止该视频的后台任务，并从当前存储后端删除索引…";
  try {
