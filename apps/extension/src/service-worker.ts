...
import { normalizeControlPlaneUrl } from "@indexed/clients/control-plane-url";
import { OSS_VECTOR_PRICING, PRODUCT_DEFAULTS, SEGMENT_SCHEMAS } from "@indexed/contracts";
import { applyVideoActivity } from "./library-state.js";
import { queueItemReadyNow } from "./queue-state.js";
import { prepareCapturedVideoForIngest } from "./video-ingest.js";
...
async function localServerRequest(config, pathname, options = {}) {
  const baseUrl = normalizeControlPlaneUrl(String(config.controlPlaneUrl || ""));
...
async function retryQueuedItemsNow() {
  const queue = await storeAll(QUEUE_STORE);
  await Promise.all(queue.map((item) => storePut(QUEUE_STORE, queueItemReadyNow(item))));
...
async function recordVideoSession(payload = {}) {
  ...
  const id = `${sourceSite}:${videoId}`;
  const optedIn = Boolean(payload.optedIn);
  const running = Boolean(payload.running);
  if (!optedIn && !running) {
    await storeDelete(SESSION_STORE, id);
...
async function ingestVisualImage(body, config) {
  ...
  const metadata = withPreview({
    ...
    embedding_input_version: embeddingCorpusInputVersion(config, "image"),
...
async function ingestVideo(body, config) {
  ...
  const vector = await embedVideo(body.video_base64, mimeType, config);
  const metadata = withPreview({
    ...
    embedding_model: config.embeddingModel,
    embedding_space: embeddingSpace(config),
    embedding_input_version: embeddingCorpusInputVersion(config, "video"),
...
async function ingestTranscript(body, config) {
  ...
  const metadata = {
    ...
    embedding_model: config.embeddingModel,
    embedding_space: embeddingSpace(config),
    embedding_input_version: embeddingCorpusInputVersion(config, "text"),
    indexed_at_ms: Date.now(),
  };
  await putVectors(
...
async function directIngest(path, body) {
  ...
  const ingestBody = kind === "video" ? await prepareCapturedVideoForIngest(body) : body;
  ...
  if (kind === "video") return ingestVideo(ingestBody, config);
  if (kind === "visual") return ingestVisualImage(ingestBody, config);
  if (kind === "transcript") return ingestTranscript(ingestBody, config);
...
async function queueVideoSegment(payload) {
  const frames = (Array.isArray(payload.videoFrames) ? payload.videoFrames : [])
    .map((frame, index) => ({
      image_base64: String(frame.imageBase64 || frame.image_base64 || ""),
      mime_type: String(frame.mimeType || frame.mime_type || "image/jpeg"),
      timestamp: Number.isFinite(Number(frame.timestamp)) ? Number(frame.timestamp) : index / 2,
    }))
    .filter((frame) => frame.image_base64);
  if (!payload.videoBase64 && !frames.length) return { error: "视频分片不能为空" };
  const segmentIndex = Math.max(0, Number(payload.segmentIndex || 0));
  const videoSegmentSchema = String(
    payload.videoSegmentSchema || (frames.length ? SEGMENT_SCHEMAS.videoFrames : SEGMENT_SCHEMAS.video)
  );
...
async function directSearch(payload) {
  ...
  if (config.backendMode === "local") {
    ...
    return {
      ...
      storage_provider: "local_zvec",
      storage_label: "本地 zvec",
  ...
  const [visualVector, transcriptVector] = await Promise.all([
    ...
    embedTranscriptQuery(query, config),
  ]);
...
function videoLibrarySummary(visualRows, transcriptRows, queue, sessions = [], storedVideos = []) {
  ...
  const ensure = (videoId, source = {}) => {
    ...
    video.last_indexed_at = Math.max(
      video.last_indexed_at,
      Number(source.indexed_at_ms || source.last_indexed_at || 0),
    );
    return video;
  };
  ...
  for (const source of storedVideos) {
    const video = ensure(source.video_id, source);
    if (!video) continue;
    video.visual_count = Math.max(video.visual_count, Number(source.visual_count || 0));
    video.transcript_count = Math.max(video.transcript_count, Number(source.transcript_count || 0));
    for (const segment of source.saved_visual_segments || source.visual_segments || []) {
      addSegment(video.saved_visual_segments, segment);
    }
    for (const segment of source.saved_transcript_segments || source.transcript_segments || []) {
      addSegment(video.saved_transcript_segments, segment);
    }
  }
  ...
  for (const session of sessions) {
    // Opening or watching a player page is not capture intent. Legacy versions
    // persisted every passive player session, so ignore those records here.
    if (!session.optedIn) continue;
...
async function deleteVideoMemory(payload = {}) {
  ...
  try {
    ...
    if (config.backendMode === "local") {
      ...
      return {
        ...result,
        visual_deleted: Number(result.visual_deleted ?? result.visualDeleted ?? 0),
        transcript_deleted: Number(result.transcript_deleted ?? result.transcriptDeleted ?? 0),
        local_deleted: queueDeleted + processedDeleted + previewsDeleted,
        stopped_tabs: stoppedTabs,
      };
    }
    const stoppedTabs = await stopOpenVideoTabs(sourceSite, videoId);
...
async function videoLibraryStatus() {
  ...
  await purgeAbandonedQueueItems();
  if (config.backendMode === "local") {
    const [library, queue, processed, sessions] = await Promise.all([
    ...
    const videos = applyVideoActivity(
      videoLibrarySummary([], [], queue, sessions, library.videos || []),
      queue,
      processed,
    );
    return {
  ...
  const videos = applyVideoActivity(
    videoLibrarySummary(libraryRowsCache.visual, libraryRowsCache.transcript, queue, sessions),
    queue,
    processed,
  );
  return {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  const run = async () => {
    ...
    if (message?.type === "GET_CAPTURE_CONFIG") {
      const config = await readConfig();
      if (config.backendMode !== "local") return { framePackets: false, transport: "webm" };
      try {
        const status = await localServerRequest(config, "/api/status");
        const framePackets = status.embeddingProvider === "apple-native";
        return { framePackets, transport: framePackets ? "ordered-frames" : "webm" };
      } catch {
        return { framePackets: false, transport: "webm" };
      }
    }
    if (message?.type === "SAVE_CONFIG") {
      ...
      const next = {
        ...
        backendMode: incoming.backendMode
          ...
          : current.backendMode,
        controlPlaneUrl: normalizeControlPlaneUrl(String(
          incoming.controlPlaneUrl || current.controlPlaneUrl || DIRECT_DEFAULT_CONFIG.controlPlaneUrl,
        )),
      ...
      const queuedForRetry = Object.prototype.hasOwnProperty.call(incoming, "backendMode")
        || Object.prototype.hasOwnProperty.call(incoming, "controlPlaneUrl")
        ? await retryQueuedItemsNow()
        : 0;
      libraryRowsCache = null;
      return {
        ok: true,
